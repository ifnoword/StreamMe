...
#define MAX_LEN_OF_SUBTITILE 40
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Event *eventItem = [self.eventList objectAtIndex:cellnum];
    
    cell.textLabel.text = eventItem.title;
    NSInteger len = eventItem.text.length;
    if (len<=MAX_LEN_OF_SUBTITILE) {
        cell.detailTextLabel.text = eventItem.text;
    }
    else{
        cell.detailTextLabel.text =[eventItem.text substringToIndex:MAX_LEN_OF_SUBTITILE];
    }
    return cell;
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    Event *event = [self.eventList objectAtIndex:selectedCellNum];
    childVC.event = event;
